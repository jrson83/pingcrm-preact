  ...
  return (
    <div class="flex items-center justify-between px-6 py-4 bg-indigo-900 md:flex-shrink-0 md:justify-center md:w-56">
      <Link class="mt-1" href="/">
        <Logo class="text-white fill-current" width="120" height="28" aria-label="App Logo" />
      </Link>
      <div class="relative md:hidden">
        <svg
          onClick={() => setMenuOpened(true)}
          class="w-6 h-6 text-white fill-current cursor-pointer"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 20 20"
        >
          <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z" />
        </svg>
        <div class={`${menuOpened ? '' : 'hidden'} absolute right-0 z-20`}>
          <MainMenu
            class="relative z-20 mt-2 pb-2 px-8 py-4 bg-indigo-800 rounded shadow-lg"
            closeMenu={() => {
              setMenuOpened(false)
            }}
          />
